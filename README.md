...
A cleaned sample of 1,000 rows is included in this repository as:
...
The remaining missing values in:
...
were intentionally kept as `NaN` because they corresponded to cancelled deliveries where these values did not logically exist.

### 3. Date and Time Parsing

The `order_date` column was converted from an `object` data type to:
...
Several categorical columns were checked for inconsistent formatting, capitalization, and extra spaces.
...
Numerical columns were checked for unrealistic or invalid values.
...
The original `gross_sales` column was preserved, while the transformed values were stored in a new column:

`gross_sales_normalized`
...
The scaled values were stored in new columns:
